    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        for (int ph = 0; ph < ehdr.e_phnum; ph++) {/* Program header table entry count */
            ...
            if(phdr.p_memsz != 0 && phdr.p_type == PT_LOAD && taskidx >= 0){
                taskinfo[taskidx].p_filesz = phdr.p_filesz;
                taskinfo[taskidx].p_memsz  = phdr.p_memsz ;
                           ...
                           short tasknum, FILE * img,int *phyaddr)
{
    ...
    printf("APP INFO START ADDR = %lx\n",APP_ADDR);
    
    int PRE_ADDR = APP_ADDR + 8 + 8 + 80 * tasknum;
    ...
    long padding_ADDR = ((PRE_ADDR % 32768 != 0 ? 1 : 0) + (PRE_ADDR / 32768))*32768;
    
    fwrite(&padding_ADDR,sizeof(unsigned long),1,img);
    (*phyaddr)+=8;
    
    printf("NUM_SECTOR = %d SECTOR_ID = %d SECTOR_offset = %d PRE_ADDR = %lx padding_ADDR = %lx\n",NUM_SECTOR,SECTOR_ID,SECTOR_offset,PRE_ADDR,padding_ADDR);

    for(int task_id = 0;task_id < tasknum; ++task_id){
        printf("TASK %d\n",task_id);
        printf("PHYADDR = %lx(HX) %d\n",(*phyaddr),(*phyaddr));
        for(int i=0;i<32;++i){
        ...
        (*phyaddr)+=8;
        fwrite(&(taskinfo[task_id].p_vaddr ) ,sizeof(unsigned long),1,img);
        (*phyaddr)+=8;        
        fwrite(&(taskinfo[task_id].p_filesz) ,sizeof(unsigned long),1,img);
        (*phyaddr)+=8;    
        fwrite(&(taskinfo[task_id].p_memsz ) ,sizeof(unsigned long),1,img);
        (*phyaddr)+=8;    
        fwrite(&(taskinfo[task_id].p_flags ) ,sizeof(unsigned long),1,img);
        (*phyaddr)+=8;            
        /*
        ...
        (*phyaddr)++;
        */
        // printf("task_id = %d HEX =  %x\n",taskinfo[task_id].TASK_ID,taskinfo[task_id].TASK_ID);
        printf("offset   = %ld HEX = %lx\n",taskinfo[task_id].offset  ,taskinfo[task_id].offset  );
        printf("size     = %ld HEX = %lx\n",taskinfo[task_id].SIZE    ,taskinfo[task_id].SIZE    );
        printf("p_vaddr  = %ld HEX = %lx\n",taskinfo[task_id].p_vaddr ,taskinfo[task_id].p_vaddr );
        printf("p_filesz = %ld HEX = %lx\n",taskinfo[task_id].p_filesz,taskinfo[task_id].p_filesz);
        printf("p_memsz  = %ld HEX = %lx\n",taskinfo[task_id].p_memsz ,taskinfo[task_id].p_memsz );
        printf("p_flags  = %ld HEX = %lx\n",taskinfo[task_id].p_flags ,taskinfo[task_id].p_flags );
        printf("================================================\n");
    }

    printf("APP INFO END ADDR = %lx(HX) %d\n",(*phyaddr),(*phyaddr));
    ...
    fseek(img,padding_ADDR,SEEK_SET);
    /* padding 4096*4KB */
    for(int i=0; i<4096*2; i++){
        for(int j=0; j<512; j++){
            unsigned long pad = 0;
            fwrite(&pad,sizeof(unsigned long),1,img);
        }
    }
}
